...
import { applicationsState, jobsState, userState } from "../store/atoms";
import { toast } from "react-toastify";
import { useJobActions,  } from "../store/jobActions.js";
...
const PostApplication = () => {
  const { singleJob } = useRecoilValue(jobsState);
  const { isAuthenticated, user } = useRecoilValue(userState);
  const { loading, error, message } = useRecoilValue(applicationsState);
  ...
  const { fetchSingleJob } = useJobActions();
  const { postApplication, clearAllApplicationErrors } = useApplicationActions();
